...
import { DocumentCore, DocumentCategory } from '../entities/document_core_entity'
...
async function main() {
  ...
  const category = await select({
    ...
    choices: Object.values(DocumentCategory).map((category) => ({ name: category, value: category })),
    ...
    await model.embedContext(async embedder => {
      for await (const splittedFile of spliter!.split(
        ...
        for (let i = 0; i < splittedFile.length; i++) {
          const chunk = splittedFile[i]!
          const docChunk = new DocumentChunk()
          docChunk.documentCore = docCore!
          ...
          try {
            ...
            logger.error(
              err,
              `Failed to embed ${i+1} chunk for document: ${docCore!.fileName}`,
            )
            continue
          }

          logger.debug(`Saving ${i+1} chunk for document: ${docCore!.fileName}`)
